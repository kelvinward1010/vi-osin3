...
import type { FlattenedItem, TreeItem, TreeItems } from "./pipeline.types";
...
function getMaxDepth({
  ...
  if (previousItem?.data?.isCollapse) {
    return previousItem.depth + 1;
  }

  if (previousItem?.parentId && findItem(items, previousItem.parentId)?.data?.isCollapse) {
