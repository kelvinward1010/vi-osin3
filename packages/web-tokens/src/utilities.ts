...
export function removeMetadata<T extends Exact<MetadataGroup, T>>(tokenGroup: T) {
  return Object.fromEntries(
...
export function getMediaConditions(breakpoints: BreakpointsTokenGroup) {
  ...
  return Object.fromEntries(
    breakpointEntries.map(
      (entry, index): [BreakpointsTokenName, BreakpointsAliasDirectionMediaConditions] => {
        const [breakpointsTokenName, breakpoint] = entry as Entry<BreakpointsTokenGroup>;
        ...
        const onlyMediaCondition =
          index === lastBreakpointIndex
            ? upMediaCondition
            : `${upMediaCondition} and ${getDownMediaCondition(breakpointEntries[index + 1][1])}`;
